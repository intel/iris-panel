...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        ct, created = orm['contenttypes.ContentType'].objects.get_or_create(model='scm', app_label='rest', name='scm')
        if created:
            perm_u, created_u = orm['auth.permission'].objects.get_or_create(content_type=ct, codename='scm_update')
            if created_u:
                perm_u.name = u'Can update scm data to IRIS'
                perm_u.save()

            perm_c, created_c = orm['auth.permission'].objects.get_or_create(content_type=ct, codename='scm_check')
            if created_c:
                perm_c.name = u'Can check scm data'
                perm_c.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        orm['contenttypes.ContentType'].objects.filter(model='scm', app_label='test').delete()
